...
represented by the symbols\( x, y, \dots \).  The syntax is
then:
\begin{align*}
  v,w :=&&& a \\
  |&&& l \\
...
The process \( \Pout{x}{v}{P} \) is an \emph{output}, which can send the value \( v \) via \( x \), then continue as \( P \).
...
All other types are \emph{linear}, \( \lin{T} \).
