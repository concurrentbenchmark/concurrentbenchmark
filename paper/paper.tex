...
In this section, we will describe the considerations we considered during the design of the benchmark challenges.
We will start with some general remarks, then describe the individual design considerations for each challenge problem.
...
We have decided not to include the choice construct in our challenge problems.
Choice constructs complicate the theory, and our aim is to keep the challenges focused.
For the same reason, each challenge problem only includes the fragment necessary to express the theorems we are interested in with regards to the challenge.
We leave extensions of the syntax for a future benchmark challenge.

Our challenge problems do not include reasoning about encodings, since we do not believe that encodings require any different reasoning techniques in a concurrency setting than in any other setting.
