...
study~\cite{AmbalLS21} develops four approaches to encoding binders in
Coq % as a first step towards developing tools for working with
in the context of higher-order process calculi.  The authors report that working directly
with de~Bruijn indices was easiest since the existing approaches
...
in textbooks (e.g.,~\cite{picalcbook}) and in existing mechanisations.
...
through different foundations and formalisms. %, enabling various approaches.
