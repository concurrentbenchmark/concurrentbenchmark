...
\begin{abstract}
  ...
  %
  For this reason, we propose a new collection of benchmark problems focused on the difficulties that typically arise when mechanising
  message-passing concurrency using process calculi.\luca{wondering whether it would make sense to say ``\dots using formal models of concurrent and distributed programming languages in the form of process calculi'' or something similar to stress that process calculi are not the subject of the challenges, but ``only'' an appropriate tool for this kind of endeavor} \dz{Maybe something like, ``\ldots that typically arise when mechanising formal models of concurrent and distributed programming languages, such as process calculi.''?} Our benchmark problems
...
calculi.\luca{see comment in abstract}
...
\section{Possible approaches}\label{sec:approaches}

\alb{Some introductory text to lead to the literature review. \sout{We may want to move this after the next Section, if we want to mirror the 3 challnges}}
\dz{Agreed, I moved it for now.} \\
\dz{Should we not also include a subsection on coinduction?} \\
\dz{I changed the name of this section to ``Possible approaches'' from ``Motivation''. Thoughts?}
...
support it.%
%
\luca{I would argue that support for recursive types and processes may also
impact the coinduction challenge. Indeed, some type systems with support for
recursion are more easily expressed \emph{coinductively}, by allowing possibly
infinite typing derivation. This application of coinduction is different from
that intended in the challenge, where it's focused on expressing notions of
process equivalence, and yet it is increasingly popular (see the related work on
infinitary linear logics) and occasionally seen with skepticism, which would
make a good case for formalization.}
