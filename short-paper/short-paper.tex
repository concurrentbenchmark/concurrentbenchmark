...
In this spirit, we propose a new collection of benchmark problems specifically designed to address typical issues that arise when mechanising message-passing concurrency using process calculi. \todo{Why just process calculi and not concurrency in general?}
While many efforts to mechanise results about process calculi have been completed (as we will discuss further below), our experience is that choosing appropriate proof mechanisation techniques and tools remains a significant challenge with a limited understanding of the trade-offs.
...
It is therefore our view that the research community in message-passing concurrency (and more specifically, process calculi) will benefit from a standardised set of challenge problems.
...
\todo{Add references to support our claims and decisions in this section.}
...
applications such as multiparty session types~\cite{10.1145/2827695,10.1145/3290343}, choreographies~\cite{DBLP:journals/jar/CruzFilipeMP23}, conversation types~\cite{DBLP:journals/tcs/CairesV10}, psi-calculi~\cite{lmcs:696}, or encodings between different calculi~\cite{DBLP:journals/iandc/Gorla10,DBLP:conf/forte/CairesP16,DBLP:journals/iandc/DardhaGS17,DBLP:conf/ecoop/ScalasDHY17,DBLP:journals/iandc/KouzapasPY19,10.1145/3479394.3479407} are not directly covered.\alb{Do we need all those citations?}
...
Linearity is of course interesting in many other sub-fields of programming languages, but we have chosen to base our challenge on a session type system primarily due to the common use of session types in concurrency theory.\alb{This is weak and somewhat tautological, as a motivation}
...
challenge since it concerns the properties of binders in restrictions.\alb{I would argue also wrt input}
It is not obvious that the techniques used to solve the original challenges about binders can be easily extended to reasoning about scope extrusion.\alb{I would say not obvious what the pros and cons are, since in fact those techniques (DB, locally nameless, HOAS, nominal) have been used.}
...
Our coinduction challenge concerns strong barbed bisimulation and congruence.\alb{we motivate why strong vs weak, but not why barbed}
Though weak barbed congruence is a more common behavioural equivalence for \(\pi\)-terms,\alb{? why terms?} here we prefer strong equivalences to avoid the need to abstract over the number of internal transitions, thus simplifying the theory.
...
\todo{Maybe cite some references in this section?}
