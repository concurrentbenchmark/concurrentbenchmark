...
to reuse. For example, the authors of~\cite{Cruz-Filipe2021b} note that the high-level parts of
...
\end{center}
%
In the future, we intend to use the website to promote best practices and tutorials derived from solutions to our challenges.
...
We begin with some general remarks, then describe the individual design considerations for each challenge problem, and the criteria for evaluating solutions thereof.
...
we want to emphasise, and omits all constructs %(such as choice) 
that
...
%\todo{Consider adding material on linearity predicates}
%
...
in~\cite{DBLP:conf/forte/ZalakainD21}.    Similarly, one
could eliminate context splitting by delegating linearity checks to a
\emph{linear predicate} defined on the process structure (\eg~\cite{BP23}). These checks 
serve as additional conditions within the typing rule. Whatever the choice,
...
\end{enumerate}
Solutions to our challenges need not strictly follow the definitions and lemmata set out in the problem text, but solutions which deviate from the original challenges must present more elaborate argumentation for their adequacy.
...
Many mechanisations need to combine techniques to handle several of these aspects at once, and some may also need to handle aspects that are not covered by our benchmark challenges at all.
...
%%% Conclusion
Ultimately, the fundamental aspects covered by our benchmark serve as the building blocks for most current research on concurrenct calculi.
It is our hope and aim that exploring solutions to our challenges will move the community closer to a future where the key basic proof techniques for concurrent calculi are as easy to mechanise as they are to write on paper.

\todo{A sentence to conclude reiterating the relevance of the challenges}
