...
Linear typing systems allow for tracking the use of resources in a program. In
the case of typed process calculi (in particular, session types), linearity is
widely used for tracking if and how a channel is used to send or receive values.
...
reduction with absence of errors.  We have chosen to only model linear (as
...
% The latter enforces two invariants, namely that no endpoint is
% used simultaneously by parallel processes and that the two endpoints of the same
% session have dual types.
...
best seen as a multi-set, most proof assistants have better support for list
...
%
A different approach is to bypass the problem of context splitting by adopting familiar ideas from algorithmic linear type checking: this is known as \emph{typing with leftovers}, as shown in~\cite{DBLP:conf/forte/ZalakainD21}.
...
%
A completely different approach is to adopt a \emph{substructural} meta-logical framework: the framework itself handles resource distribution, and users need only map their linear operations to the ones offered by the framework.
...
A compromise is the so called \emph{two-level} approach, \ie encoding a substructural specification logic in a mainstream proof assistant and then using that logic to state and prove linear properties (for a recent example, see~\cite{Felty:MSCS21}).
