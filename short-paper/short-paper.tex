...
Concurrent calculi are notably subtle: for instance it took nine years before the incorrect proof of type safety in the seminal paper~\cite{Honda1998} was fixed in~\cite{Yoshida2007}.
Similarly, many key results in peer-reviewed papers on session types have turned out to be false later on~\cite{Gay2020,10.1145/3290343}, illustrating the value of machine-checkable proofs.
While results about concurrent formalisms have already been mechanised (as we will discuss further below), our experience is that choosing appropriate mechanisation techniques and tools remains a significant challenge and that the trade-offs are not well understood.
...
\cite{Cruz-Filipe2021b} notes that the high-level parts of mechanised proofs closely resemble the informal versions, and that the main challenge lies in getting the foundations right.
...
Mechanisations must often address several of these aspects, but we see the combination of techniques as a next step beyond the scope of this challenge, as discussed further in \Cref{sec:going-beyond}.
...
is notoriously difficult: \lore{add here}
\cite{Castro-Perez2021} note that the most challenging
