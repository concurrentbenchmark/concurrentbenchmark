...
In this spirit, we introduce a novel collection of benchmark problems
specifically crafted to tackle common challenges encountered during
the mechanization of formal models for concurrent and distributed
...
% languages in general, and process calculi in particular. 
...
% for instance it took nine years before the incorrect proof of type
% safety in the seminal paper by Honda et al was fixed
% in~\cite{Yoshida2007}.
Similarly, many
...
% focus on three key aspects which often cause difficulties when mechanising concurrency theory:
...
discussed further in \Cref{sec:going-beyond}.  Our benchmark
...
% exhibit of typical issues encountered in mechanisation work.
...
% The three aspects are certainly not the only causes of difficulties,
% but as we will discuss presently, they emerge in most mechanisations
...
\alb{I really do not like the duplication of the discussion of the
  three issues here and in Sec 2. I understand the desire to highlight
  noted issues, but it does not work. Some citations are unconvincing. Mostly, we are anticipating the description of the challenges. Let's unify this}
\paragraph{Linearity.}
...
\lore{I have tried this:
  the first paragraph motivates (``why is this difficult?''), while
  the second talks about the challenge.}
...
is non-uniform across development.
...
%% (observable) behaviour of process calculi, both
%% in classical textbooks (e.g., \cite{picalcbook})
%% and in existing 
%% mechanisations
%% \cite{Thiemann2019,Honsell2001,Bengtson2016,Kahsai2008,Gay2020,
%% Hirschkoff1997,Bengtson2009,Maksimovic2015}.

%% , where we see a variety of
%% different formalisation choices.
%% As a significant example, we observe that,
%% while many authors rely
%% on the native support for coinduction offered by
%% proof assistants
%% \cite{Thiemann2019,Honsell2001,Bengtson2016,Kahsai2008,Gay2020},
%% many others prefer a more ``set-theoretic''
%% approach
%% \cite{Hirschkoff1997,Bengtson2009,Maksimovic2015}.
...
\alb{This subsection does not make a lot of sense here -- introductions do not have subsections. Let's move it in ``Design'' }
...
calculi~\cite{DBLP:journals/iandc/Gorla10,%,DBLP:conf/forte/CairesP16,DBLP:journals/iandc/DardhaGS17,
  DBLP:conf/ecoop/ScalasDHY17}
  %,DBLP:journals/iandc/KouzapasPY19,10.1145/3479394.3479407}
are not directly covered.

\alb{I've commented some citations about
  encodings. Is it a good idea to start the section describing what we \textbf{do not do}?}

Still, these (and other) applications need the basic
techniques that our challenge problems emphasize:
Aligned with design goal~\ref{item:goal-reusability}, our problems are rooted in the fundamental meta-theory of process calculi, without necessitating the creation of novel theory. These problems center around well-established theorems and results, showcasing intriguing proof techniques that can be leveraged in future endeavors.
% as per design goal
...
% interesting proof techniques that may be reused in further work. 
To
...
may be solved individually and in any order; each problem is %should be
...
\alb{I would put the challenge before the literature}
...
A good example of this are the parameterized coinduction~\cite{Hur2013}
...
\todo{Insert discussion/related work focusing on existing techniques and how our benchmark design makes it possible to compare them.}
\lore{I have added something below... Any advice on what more we need?}
...
\alb{This should not be a subsubsection at the same level of coinduction etc}
...
challenges should be compared on three axis:
\begin{enumerate}
