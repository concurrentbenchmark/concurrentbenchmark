...
In this spirit, we propose a new collection of benchmark problems specifically designed to address typical issues that arise when mechanising message-passing concurrency using process calculi.
...
Our experience, however, is that choosing the appropriate proof mechanisation techniques and 
...
These are certainly not the only factors that may cause issues, but they are fundamental to concurrency theory, and emerge in most mechanisations.

\todo{Rewrite this section to be more coherent.}
...
as a next step beyond the scope of this challenge, as discussed further in \Cref{sec:going-beyond}.
...
applications such as multiparty session types~\cite{10.1145/2827695,10.1145/3290343}, choreographies~\cite{DBLP:journals/jar/CruzFilipeMP23}, conversation types~\cite{DBLP:journals/tcs/CairesV10}, psi-calculi~\cite{lmcs:696}, or encodings between different calculi~\cite{DBLP:journals/iandc/Gorla10,DBLP:conf/forte/CairesP16,DBLP:journals/iandc/DardhaGS17,DBLP:conf/ecoop/ScalasDHY17,DBLP:journals/iandc/KouzapasPY19,10.1145/3479394.3479407} are not directly covered.\alb{Do we need all those citations?}  Still, these (and other) applications need the basic techniques that our challenge problems emphasize:
...
Though weak barbed congruence is a more common behavioural equivalence for \(\pi\)-terms,\alb{? why terms?} here we prefer strong equivalences to avoid the need to abstract over the number of internal transitions, % when considering the strong notions of equivalence,
