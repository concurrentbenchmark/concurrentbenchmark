...
Our benchmark problems (detailed in \cref{app:challenges} and on the website) consider
...
% \todo{Mention the higher-order pi calculus? If so, also
%   mention}


Like the POPLMark and POPLMark Reloaded challenges, our % challenge
is
...
message-passing calculi %to be explored in further challenges could be
include
...
calculi~\cite{DBLP:journals/iandc/Gorla10,
  DBLP:conf/ecoop/ScalasDHY17}.\todo{Should we cite other/different
  encoding papers? Which ones are the ``standard'' citations?}
...
% \todo{This is a list of interesting research problems, but they are
%   not challenges per se. Trim down.}  On the mechanisation side, the
% present challenge proposes mechanising the proofs of theorems.
...
protocol.
% a more specific
% challenge could be designed with the objective of automating aspects
% of the proofs.
