...
meta-logical framework: the framework itself handles resource
distribution, and users need only map their linear operations to the
ones offered by the framework.  The linear function space will then
handle all context operations implicitly, including splitting and
substitution.  The only such framework is
