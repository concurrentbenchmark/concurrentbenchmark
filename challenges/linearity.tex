...
%%  last applied rule of the proof of --AM changed for uniformity
  ...
  \item[\TirName{T-End}] Since $\tend{\Delta}$, we can just reapply
    the rule without $x:T$.
    % [$P = \Pend$] we Apply \TirName{T-Inact}.
    
  \item[\TirName{T-Par}] In this case, we have that
    $ \Delta, x : T = \Delta_0 , \Delta_1$.  By cases on which
    environment $x$ is in, we just apply the induction hypothesis on
    that environment.
    %
    %
  ...
  \item[\TirName{T-Res}] Without loss of generality, we assume that
    ...
    the assumptions present we can apply the induction hypothesis.
    % inversion,
    % $\Delta, x : T = \Delta', y : T_0, z : \dual{T_1}, x : T $. From
...
\begin{proof}
  ...
  $y_1$ such that $\scong{P_1}{\Pout{x_1}{v}{P_1'}}$ (the opposite case
  proceeds similarly). We need to show that
  $\scong{P_2}{\Pin{y_1}{v}{P_2'}}$. This can be easily done by
  contradiction. In fact, if $\scong{P_2}{\Pout{x}{v}{P_2'}}$ then the
  typing rule for restriction would be violated since the type of
