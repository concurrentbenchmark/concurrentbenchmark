...
\end{lemma}
\begin{proof}
  By induction on $P$, for each case inverting $\types{\Gamma; (\Delta, x : T)}{P}$ to the uniquely applicable rule.
  \begin{description}
    \item[$P = \Pend$] By inversion on the typing we have $\tend{\Delta}$. Apply T-INACT.
    \item[$P = \Ppar{P_0}{P_1}$] By inversion on the typing we have  $ \Delta, x : T = \Delta_0 , \Delta_1$.
      By cases on which environment $x$ is in, apply IH on that environment. 
     \item[$P = \Presd{y}{z}{P}$] We may assume $x \notin \{y,z\}$. By inversion, $\Delta, x : T = \Delta', y : T_0, z : \dual{T_1}, x : T $. From the assumptions present we can apply IH.\\

       The remaining case are analogous
...
\end{lemma}
\begin{proof}
  By induction on $P$ inverting  $\types{(\Gamma, l);\Delta}{P}$ in each case.
  \begin{description}
    \item[$P = \Pend$] Immediate after inverting typing.
    \item[$P = \Ppar{P_0}{P_1}$] By inversion on the typing we get derivations for $P_0$ and $P_1$. Apply IH on both.
     \item[$P = \Presd{x}{y}{P}$] Immediate after inverting typing.
     \item[$P = \Pout{x}{v}{P}$] Invert typing. we have  $\typev{(\Gamma,l)}{\hastype{v}{\Tbase}}$.
       We must show $\typev{\Gamma}{\hastype{v}{\Tbase}}$ to build the conclusion with IH and T-OUT. Go by cases on the struture of $\typev{(\Gamma,l)}{\hastype{v}{\Tbase}}$.
...
\end{theorem}
\begin{proof}
  By induction on the derivation of $\reduces{P}{Q}$. The cases R-PAR
  and R-RES follows immediately by IH. Case R-STRUCT appeals twice to
  preservation of $\equiv$ (Lemma~\ref{le:presequiv}) and to the
  IH. For R-COM, suppose that T-RES introduces in $\Delta$ the
  assumptions $x : !.U, y : ?\dual U$.  Building the only tree for the
  hypothesis, we know that $\Delta = \Delta_1, \Delta_2, \Delta_3$
  where $\Gamma;\Delta_3 \vdash R$. We also have
  $ \Gamma;\Delta_1 , x : U \vdash P$, ${\cal D}_{2} $ a proof of
  $\Gamma,l;\Delta_2, y : \dual U \vdash Q$ and $\cal V $ is a proof of
  $\Gamma \vdash a : \Tbase$.  From ${\cal D}_{2}$ and $\cal V$ we use the
  substitution Lemma~\ref{le:subst} to obtain $\Gamma;\Delta_2 , y : \dual U \vdash
  Q[a/l]$. We then conclude the proof with rules T-PAR, T-PAR and
  T-RES.
\end{proof}
...
and \( P_2 \) is prefixed at \( y_1 \) (or viceversa), then
\( \Ppar{P_1}{P_2}
...
\begin{proof}
  ...
  structurally congruent to $P$. Moreover, assume that $P_1$ is
  prefixed at some $x_1$ and $P_2$ is prefixed at some $x_2$ such that
  $P_1\equiv\Pout{x}{v}{P_1'}$ (the opposite case proceeds
  similarly). We need to show that $P_2\equiv\Pin{x}{v}{P_2'}$. This
  can be easily done by contradiction. In fact, if
  $P_2\equiv\Pout{x}{v}{P_2'}$ then the typing rule for restriction
  would be violated since the type of $x_1$ and $y_1$ cannot be dual.
  % 
