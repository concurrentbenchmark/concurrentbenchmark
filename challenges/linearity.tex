...
\begin{proof}
  ...
  \item[\TirName{Res-Par}] By case analysis on $x : T$ being linear or $\tend$ and applying weakening and strengthening accordingly.
  \item[\TirName{Res-Inact:}] Use weakening~\ref{lemma:weak} part 2
  \item[\TirName{Res}] Noting that order does not matter
  \end{description}
\end{proof}

Now, following Sangiorgi \& Walker define as the compatible equivalence relation induced by $\sconga\cdot\cdot$, namely add to reflexivity, symmetry  and transitivity the following:
 \begin{mathpar}
