...
%%  last applied rule of the proof of --AM changed for uniformity
  ...
  \item[\TirName{T-Res}] Without loss of generality, we assume that
    ...
    $\types{\Gamma;\;\Delta, y : T_0, z : \dual{T_0}, x : T}{P'}$, by
    induction hypothesis, we have
    $\types{\Gamma;\;\Delta, y : T_0, z : \dual{T_0}}{P'}$. Applying
    again \TirName{T-Res}, we have
    $\types{\Gamma;\;\Delta}{\Presd yz{P'}}$.
    % inversion,
...
\begin{lemma}[Substitution]\mbox{}
  \label{le:subst}
  If $\types{(\Gamma, l);\Delta}{P}$ and
  ${\typev{\Gamma}{\hastype{a}{\Tbase}}}$ then
  \( \types{\Gamma;\Delta}{\subst{P}{a}{l}} \).
\end{lemma}
\begin{proof}
  By induction on the derivation of $\types{(\Gamma, l);\Delta}{P}$.
  \begin{description}
