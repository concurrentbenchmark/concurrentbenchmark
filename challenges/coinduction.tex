...
semantics which features less rules but needs a structural
...
builds on a notion of \emph{observable}, i.e., what can we externally
observe from the semantics of a process and can we use that to tell
two different process apart.  If we allow ourselves only to observe
...
In order to make sure that strong barbed bisimilarity detects cases
like the one above, we need to restrict it so that it becomes a
congruence,
i.e., % we need to make strong barbed bisimilarity consider
it has to consider the environment in which processes may be placed.
